...
class PresensiMasukFragment : Fragment(), OnMapReadyCallback {
            ...
            .addOnSuccessListener { location: Location? ->
                location?.let {
                    ...
                    if (!addresses.isNullOrEmpty()) {
                        ...
                        lokasiMasuk.text = address
                    } else {
                        lokasiMasuk.text = "Tidak dapat mendeteksi lokasi"
                    }
        ...
        lifecycleScope.launch {
            try {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    Log.e("PresensiMasuk", "Error: ${response.errorBody()?.string()}")
                }
            } catch (e: Exception) {
                Toast.makeText(requireContext(), "Kesalahan: ${e.message}", Toast.LENGTH_SHORT).show()
                Log.e("PresensiMasuk", e.stackTraceToString())
            }
        }
    }
    ...
    private val startForResult =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            if (result.resultCode == Activity.RESULT_OK) {
                ...
                imageFile = bitmapToFile(photo, "presensi_masuk.jpg")
            }
        }
    ...
    // Kirim data presensi masuk ke API Laravel
